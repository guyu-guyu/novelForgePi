...
- `prompts/new-book.md` 的 `/new-book` 流程只创建书骨架与隔离目录，**没有把题材包复制或
  symlink 到书项目**。
...
### 7. 隔离模式不能在运行中的 pi 里切换

`isolatePi.sh` 通过 `export PI_CODING_AGENT_DIR=...` 后 `exec pi`（`:75-77`）启动新进程。
`PI_CODING_AGENT_DIR` 必须在 pi 启动前设置；运行中的 pi 会话无法切换全局配置目录
（`/reload` 不重载 auth/models/settings）。

- `/new-book` 只能"备料"（创建 `.pi-isolated/` 和骨架），真正进入隔离模式必须退出当前 pi，
  在书项目目录跑 `./isolatePi.sh`。
- 会话内无法热切换，体验上有断裂感。
...
- **题材包落地**：`/new-book` 时 symlink 或 copy 题材包到书项目的 `genre-packs/`；或让
  `/genre-pack use` 在改 `book.md` 字段的同时复制文件。确保 `scene-writing` 技能能在书项目
...
- **会话内隔离切换**：研究能否用 `pi.registerProvider` + `setActiveTools` 在会话内动态切换
  配置，绕过 `PI_CODING_AGENT_DIR` 的进程级限制。或提供 `/isolate` 命令直接 `exec
  isolatePi.sh`（重启会话但省去用户记忆脚本路径）。
